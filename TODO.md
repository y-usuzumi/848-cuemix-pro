...
- [ ] AVDECC push updates: the 848 accepts an HTTP `CONNECT` tunnel on TCP 17221 and advertises proxy `Version=1`. Map that version-1 handshake before transmitting AECP; then enumerate descriptors, safely register for unsolicited notifications, renew the registration when required, and retain HTTP polling as a recovery fallback.
