...
function fixture() {
  ...
  for(let i=0;i<12;i++) add(0x93ac,i,M.path(0x13ad,1,i%2));
...
if(require.main===module) {
  let snapshot=fixture(), requests=[], failNext=false;
  ...
  http.createServer((req,res)=> {
    ...
    if(req.method==='GET') {
      ...
      if(url.pathname==='/api/mixer/meters/events') {res.writeHead(200,{'Content-Type':'text/event-stream'});return res.end('data: '+JSON.stringify({records:[{property_id:'13ad',index:0,channels:[48,48,96,96]},{property_id:'13ad',index:1,channels:[24,24]}]})+'\n\n');}
    ...
    let body='';req.on('data',c=>body+=c);req.on('end',()=> {
      ...
      requests.push(edits);json(200,{acknowledged:edits.length});
    });
